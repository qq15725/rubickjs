...
export class HTMLSound extends EventEmitter implements ISound {
  ...
  set(name: 'playbackRate' | 'volume' | 'muted' | 'loop' | 'paused', value: number | boolean): this {
    if (this[name] === undefined) {
      ...
      switch (name) {
        case 'playbackRate': this.playbackRate = value as number; break
  ...
  get processors(): Array<AudioProcessor> {
    console.warn('HTML Audio does not support processors')
    return []
  }

  set processors(_val: Array<AudioProcessor>) {
    console.warn('HTML Audio does not support processors')
  }

  refresh(): void {
    if (!this._audio || !this._source) return
  ...
  refreshPaused(): void {
    if (!this._source || !this._audio) return
