...
export class Line {
  static buildGeometry(
    ...
    const widthSquare = widthHalf * widthHalf
    const miterLimitSquare = miterLimit * miterLimit
    ...
    for (let i = 1; i < length - 1; ++i) {
      ...
      const smallerInsideDiagonalSq = smallerInsideSegmentSq + (insideWeight * insideWeight * widthSquare)
      const insideMiterOk = pdist <= smallerInsideDiagonalSq

      if (join === 'miter' && pdist / widthSquare > miterLimitSquare) {
        join = 'bevel'
      }
    ...
    if (!closed) {
      if (cap === 'round') {
        ...
        indexCount += this._square(x1, y1, perpx, perpy, innerWeight, outerWeight, false, vertices)
      }
    }
