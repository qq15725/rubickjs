...
const RE1 = /^uniform \w+ (.+?);\s*\/\/\s*=\s*([^;]+?)[\s;]*$/gm
...
function parseVal(val: string): number | Array<number> {
  val = val.trim()
...
void main() {
  ...
  constructor(glsl: string) {
    ...
    const has = {} as Record<keyof typeof EffectMaterial.RE, boolean>
...
${ glsl }${
    ...
    const matched = glsl.matchAll(RE1)
